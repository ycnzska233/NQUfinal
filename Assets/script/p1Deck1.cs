...
public class p1Deck1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (decks.p1s1.Count != 0)
        {
            gameObject.GetComponent<SpriteRenderer>().sprite = value[decks.p1s1[decks.p1s1.Count - 1] - 2];
            if (decks.p1s1.Count > 1)
            {
                card.SetActive(true);
                card.GetComponent<SpriteRenderer>().sprite = value[decks.p1s1[decks.p1s1.Count - 2] - 2];
            }
    ...
    void OnMouseUp()
    {
        if (decks.target == true && guide.activeSelf == true)
        {
            ...
            {
                if (decks.player1[i].suit == decks.suit[1])
                {
                    if (decks.player1[i].value == decks.p1s1[decks.p1s1.Count - 1]) { index = i; }
                }
            }
