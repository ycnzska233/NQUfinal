...
public class p2Deck6 : MonoBehaviour
{
    ...
    void Update()
    {
        if (decks.p2s6.Count != 0)
        {
            gameObject.GetComponent<SpriteRenderer>().sprite = value[decks.p2s6[decks.p2s6.Count - 1] - 4];
            if (decks.p2s6.Count > 1)
            {
                card.SetActive(true);
                card.GetComponent<SpriteRenderer>().sprite = value[decks.p2s6[decks.p2s6.Count - 2] - 4];
            }
    ...
    void OnMouseUp()
    {
        if (decks.target == true && guide.activeSelf == true)
        {
            ...
            {
                if (decks.player2[i].suit == decks.suit[6])
                {
                    if (decks.player2[i].value == decks.p2s6[decks.p2s6.Count - 1]) { index = i; }
                }
            }
