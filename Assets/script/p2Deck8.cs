...
public class p2Deck8 : MonoBehaviour
{
    ...
    void Update()
    {
        if (decks.p2s8.Count != 0)
        {
            gameObject.GetComponent<SpriteRenderer>().sprite = value[decks.p2s8[decks.p2s8.Count - 1] - 2];
            if (decks.p2s8.Count > 1)
            {
                card.SetActive(true);
                card.GetComponent<SpriteRenderer>().sprite = value[decks.p2s8[decks.p2s8.Count - 2] - 2];
            }
    ...
    void OnMouseUp()
    {
        if (decks.target == true && guide.activeSelf == true)
        {
            ...
            {
                if (decks.player2[i].suit == decks.suit[8])
                {
                    if (decks.player2[i].value == decks.p2s8[decks.p2s8.Count - 1]) { index = i; }
                }
            }
