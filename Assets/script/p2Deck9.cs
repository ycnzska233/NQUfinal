...
public class p2Deck9 : MonoBehaviour
{
    ...
    void Update()
    {
        if (decks.p2s9.Count != 0)
        {
            gameObject.GetComponent<SpriteRenderer>().sprite = value[decks.p2s9[decks.p2s9.Count - 1] - 2];
            if (decks.p2s9.Count > 1)
            {
                card.SetActive(true);
                card.GetComponent<SpriteRenderer>().sprite = value[decks.p2s9[decks.p2s9.Count - 2] - 2];
            }
    ...
    void OnMouseUp()
    {
        if (decks.target == true && guide.activeSelf == true)
        {
            ...
            {
                if (decks.player2[i].suit == decks.suit[9])
                {
                    if (decks.player2[i].value == decks.p2s9[decks.p2s9.Count - 1]) { index = i; }
                }
            }
